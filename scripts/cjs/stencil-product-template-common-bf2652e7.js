...
function makeMatchConditions(mustMatch, mustNotMatch) {
    ...
    for (const field in mustMatch) {
        ...
        conditions.push(commerce.ProductTemplatesHelpers.fieldMustMatch(field, mustMatch[field]));
    }
    for (const field in mustNotMatch) {
        conditions.push(commerce.ProductTemplatesHelpers.fieldMustNotMatch(field, mustNotMatch[field]));
    }
    return conditions;
}
...
function makeDefinedConditions(ifDefined, ifNotDefined) {
    ...
    if (ifDefined) {
        ...
        conditions.push(commerce.ProductTemplatesHelpers.fieldsMustBeDefined(fieldNames));
    }
    if (ifNotDefined) {
        const fieldNames = ifNotDefined.split(',');
        conditions.push(commerce.ProductTemplatesHelpers.fieldsMustNotBeDefined(fieldNames));
    }
    return conditions;
}

exports.ProductTemplateCommon = ProductTemplateCommon;
exports.makeDefinedConditions = makeDefinedConditions;
exports.makeMatchConditions = makeMatchConditions;

//# sourceMappingURL=stencil-product-template-common-bf2652e7.js.map