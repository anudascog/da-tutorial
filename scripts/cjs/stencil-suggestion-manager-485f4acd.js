...
const SearchBoxWrapper = (props, children) => {
    ...
    return (index.h("div", { part: "wrapper", class: getClasses(), onFocusout: props.onFocusout }, children));
};
...
const TextAreaClearButton = ({ inputRef, bindings, onClick, ...defaultButtonProps }) => (index.h("div", { part: "clear-button-wrapper", class: "ml-2 flex items-center justify-center py-2" },
    index.h(stencilButton.Button, { style: "text-transparent", part: "clear-button", class: "text-neutral-dark flex h-8 w-8 shrink-0 items-center justify-center", onClick: () => {
            onClick?.();
            inputRef?.focus();
        }, ariaLabel: bindings.i18n.t('clear'), ...defaultButtonProps },
        index.h("atomic-icon", { part: "clear-icon", icon: ClearSlim, class: "h-4 w-4" }))));
...
const SearchTextArea = ({ textAreaRef, loading, bindings, onInput, onFocus, onBlur, onChange, onKeyDown, onKeyUp, value, ariaLabel, onClear, popup, ...defaultInputProps }) => (index.h("div", { class: "flex grow overflow-hidden" },
    index.h("div", { part: "textarea-expander", class: "grid grow overflow-hidden" },
    ...
    loading && (index.h("div", { class: "searchbox-button-wrapper ml-2 flex items-center justify-center" },
        index.h("span", { part: "loading", class: "loading mr-2 grid h-5 w-5 animate-spin place-items-center rounded-full bg-linear-to-r" }))),
...
class SuggestionManager {
    ...
    isRightPanelInFocus() {
        if (bueno.isNullOrUndefined(this.panelInFocus) ||
            bueno.isNullOrUndefined(this.rightPanel)) {
            return false;
        }
    ...
    async executeQuerySuggestion() {
        ...
        const defaultSuggestedQuery = this.allSuggestionElements.find(suggestionsCommon.elementHasQuery)?.query || '';
    ...
    isPanelInFocus(panel, query) {
        ...
        if (query) {
            const escaped = purify.DOMPurify.sanitize(query);
    ...
    getAndFilterLeftSuggestionElements() {
        ...
        const out = suggestionElements.filter((suggestionElement) => {
            if (bueno.isNullOrUndefined(suggestionElement.query)) {
                return true;
            }
...
//# sourceMappingURL=stencil-suggestion-manager-485f4acd.js.map