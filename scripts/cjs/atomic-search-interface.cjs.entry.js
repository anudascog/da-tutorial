...
const searchLayout = require('./search-layout-f66ff241.js');
const analyticsConfig = require('./analytics-config-04b5842e.js');
...
require('./search-layout-ced12c1e.js');
...
function createSearchStore() {
    const store$1 = store.createBaseStore({
        loadingFlags: [],
        iconAssetsPath: '',
    ...
    return {
        ...
        unsetLoadingFlag(loadingFlag) {
            store.unsetLoadingFlag(store$1, loadingFlag);
        },
        setLoadingFlag(loadingFlag) {
            store.setLoadingFlag(store$1, loadingFlag);
        },
        isMobile() {
            return !window.matchMedia(searchLayout.makeDesktopQuery(store$1.state.mobileBreakpoint))
                .matches;
        },
        registerFacet(facetType, data) {
            store.registerFacet(store$1, facetType, data);
        },
        getFacetElements() {
            return store.getFacetElements(store$1);
        },
        waitUntilAppLoaded(callback) {
            store.waitUntilAppLoaded(store$1, callback);
        },
        getUniqueIDFromEngine(engine) {
            return engine.state.search.response.searchUid;
        },
        hasLoadingFlag(loadingFlag) {
            return store$1.state.loadingFlags.indexOf(loadingFlag) !== -1;
        },
        getAllFacets() {
            return {
                ...store$1.state.facets,
                ...store$1.state.dateFacets,
                ...store$1.state.categoryFacets,
                ...store$1.state.numericFacets,
            };
        },
        addFieldsToInclude(fields) {
            const currentFields = store$1.state.fieldsToInclude;
            store$1.state.fieldsToInclude = [...currentFields, ...fields];
        },
    };
}
...
const AtomicSearchInterface = class {
    constructor(hostRef) {
        ...
        this.commonInterfaceHelper = new interfaceCommonStencil.CommonAtomicInterfaceHelper(this, 'CoveoAtomic');
        ...
        this.i18n = interfaceCommonStencil.instance.createInstance(undefined, resolve);
    }
    connectedCallback() {
    ...
    updateSearchConfiguration(updatedProp, newValue) {
        ...
        const { updateSearchConfiguration } = headless.loadSearchConfigurationActions(this.engine);
    ...
    initializeWithSearchEngine(engine) {
        if (this.pipeline && this.pipeline !== engine.state.pipeline) {
            console.warn(interfaceCommonStencil.mismatchedInterfaceAndEnginePropError('search', 'query pipeline'));
        }
        if (this.searchHub && this.searchHub !== engine.state.searchHub) {
            console.warn(interfaceCommonStencil.mismatchedInterfaceAndEnginePropError('search', 'search hub'));
        }
    ...
    async executeFirstSearch() {
        ...
        const { updateQuery } = headless.loadQueryActions(this.engine);
    ...
    initFieldsToInclude() {
        const fields = headless.EcommerceDefaultFieldsToInclude.concat(this.fieldsToInclude);
        this.store.addFieldsToInclude(fields);
    }
    registerFieldsToInclude() {
        this.engine?.dispatch(headless.loadFieldActions(this.engine).registerFieldsToInclude(this.store.state.fieldsToInclude));
    }
    ...
    initEngine(options) {
        ...
        try {
            this.engine = headless.buildSearchEngine({
                configuration: {
                    ...options,
                    search: searchConfig,
    ...
    initUrlManager() {
        ...
        this.urlManager = headless.buildUrlManager(this.engine, {
    ...
    initSearchStatus() {
        this.searchStatus = headless.buildSearchStatus(this.engine);
        this.unsubscribeSearchStatus = this.searchStatus.subscribe(() => {
            const hasNoResultsAfterInitialSearch = !this.searchStatus.state.hasResults &&
                this.searchStatus.state.firstSearchExecuted &&
                !this.searchStatus.state.hasError;
            this.host.classList.toggle(searchLayout.noResultsSelector, hasNoResultsAfterInitialSearch);
            this.host.classList.toggle(searchLayout.errorSelector, this.searchStatus.state.hasError);
            this.host.classList.toggle(searchLayout.firstSearchExecutedSelector, this.searchStatus.state.firstSearchExecuted);
    ...
    render() {
        return [
            this.engine && this.enableRelevanceInspector && (index.h("atomic-relevance-inspector", { key: 'fedd278006eb7ba151931df2464c3edf8a91e844', open: this.relevanceInspectorIsOpen, bindings: this.bindings })),
            index.h("slot", { key: '1837df3ca11a36d9e2b88b4fc32d0f8a5a66b1e6' }),
        ];
    }
