...
class DisplayIfDirective extends Directive {
    render(_condition, _children) {
        return noChange;
    }
    update(part, [displayCondition, children]) {
        if (part.type !== PartType.CHILD) {
            throw new Error(`"displayIf" can only be used in child bindings. Expected PartType "${PartType.CHILD}", but received PartType "${part.type}".`);
        }
