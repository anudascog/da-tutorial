...
export declare const tw: <T extends {
    [x: string]: boolean;
}>(rec: NoDuplicateWords<T>) => NoDuplicateWords<T>;
/**
 ...
 */
type Split<Word extends string, Separator extends string, Limit extends number = 30, Count extends unknown[] = []> = Count['length'] extends Limit ? ['$$recursion_limit_reached_flag$$'] : Word extends `${infer PreviousWord}${Separator}${infer NextWord}` ? [PreviousWord, ...Split<NextWord, Separator, Limit, [unknown, ...Count]>] : [Word];
/**
...
type WordMap<T extends Record<string, boolean>> = {
    [K in keyof T]: Extract<Split<K & string, ' '>[number], '$$recursion_limit_reached_flag$$'> extends never ? Split<K & string, ' '>[number] : '$$recursion_limit_reached_flag$$';
};
/**
...
type DuplicateWords<T extends Record<string, boolean>> = {
    [K in keyof T]: WordMap<T>[K] extends '$$recursion_limit_reached_flag$$' ? false : Extract<WordMap<T>[K], WordMap<T>[Exclude<keyof T, K>]>;
}[keyof T];
/**
 ...
 */
type NoDuplicateWords<T extends Record<string, boolean>> = string extends keyof T ? T : DuplicateWords<T> extends never ? T : DuplicateWords<T> extends false ? 'Recursion limit reached in class map. Ensure the class list is not too long.' : 'Duplicate class detected in class map. Ensure that no class appears in more than one key.';
declare class MultiClassMapDirective extends Directive {
    /**
     ...
     */
    render(classInfo: Record<string, boolean>): import("lit-html/directive.js").DirectiveResult<typeof import("lit-html/directives/class-map.js").ClassMapDirective>;
}
export declare function multiClassMap<T extends Record<string, boolean>>(record: NoDuplicateWords<T>): import("lit-html/directive.js").DirectiveResult<typeof MultiClassMapDirective>;
export {};
